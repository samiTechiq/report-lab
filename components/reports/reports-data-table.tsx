...
export function ReportsDataTable() {
	...
	// New state to store ALL filtered reports for aggregation
	const [allFilteredReports, setAllFilteredReports] = useState<Report[]>([]);
	const [aggregationLoading, setAggregationLoading] = useState(false);

	// Function to load all filtered reports for both CSV and aggregation
	const loadAllFilteredReports = async () => {
		try {
			...
			setAllFilteredReports(reports);
			return reports;
		} catch (error) {
			console.error("Error loading all reports for aggregation:", error);
			return [];
		} finally {
	...
	const { data, isLoading, error, isFetching } = useQuery({
		...
		queryFn: async () => {
			...
			// If this is the first page and we have filters, also load all data for aggregation
			if (
				currentPage === 1 &&
				(filters.startDate || filters.endDate || filters.location)
			) {
				loadAllFilteredReports();
			}
	...
	const handleFilter = (newFilters: {
		...
		// Clear the all reports array when filters change
		// The data will be loaded on the first page query
		setAllFilteredReports([]);
	};

	const handleExportCSV = async () => {
		try {
			let reports = allFilteredReports;

			// If we don't have the full dataset yet, fetch it
			if (reports.length === 0) {
				reports = await loadAllFilteredReports();
			}
				...
				<div className="flex flex-col space-y-4">
					...
					<div className="space-y-2">
						...
						<div className="flex justify-between items-center text-sm">
							...
							<span className="ml-2">
								{formatNumber(report?.opening_kg)}
							</span>
						</div>
						<div className="flex justify-between items-center text-sm">
						...
						<div className="flex justify-between items-center text-sm">
							...
							<span className="ml-2">
								{formatNumber(report.kg_used)}
							</span>
						</div>
						<div className="flex justify-between items-center text-sm">
							<span className="font-medium">Closing:</span>
							<span className="ml-2">
								{formatNumber(report.closing_kg)}
							</span>
						</div>
					</div>
					...
					<div className="space-y-2">
						...
						<div className="flex justify-between items-center text-sm">
							<span className="font-medium">Opening:</span>
							<span className="ml-2">
								{formatNumber(report.opening_bag)}
							</span>
						</div>
						<div className="flex justify-between items-center text-sm">
							<span className="font-medium">Produced:</span>
							<span className="ml-2">
								{formatNumber(report.bag_produced)}
							</span>
						</div>
						<div className="flex justify-between items-center text-sm">
							<span className="font-medium">Sold:</span>
							<span className="ml-2">
								{formatNumber(report.bag_sold)}
							</span>
						</div>
						<div className="flex justify-between items-center text-sm">
							<span className="font-medium">Closing:</span>
							<span className="ml-2">
								{formatNumber(report.closing_bag)}
							</span>
						</div>
						<div className="flex justify-between items-center text-sm">
							<span className="font-medium">Missing:</span>
							<span className="ml-2">
								{formatNumber(report.missing_bag)}
							</span>
						</div>
					</div>
						...
						{isLoading ? (
							...
							filteredReports.map((report) => (
								<TableRow key={report.id}>
									...
									<TableCell>
										{subStr(report.location, 10)}
									</TableCell>
									<TableCell>
										{subStr(report.recorded_by, 12)}
									</TableCell>
									<TableCell>
										<EditReportDialog report={report} />
									</TableCell>
